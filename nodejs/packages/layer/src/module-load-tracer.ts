...
const traceRequire = function (moduleId: string): unknown {
  // @ts-expect-error to make linter happy
  const scopeObj = this;
  if (!activated) {
    return originalRequire.call(scopeObj, moduleId);
  }
  let originalRequireCalled = false;
  let res: unknown;
  try {
